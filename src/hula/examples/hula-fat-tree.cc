...
main(int argc, char *argv[])
{
	...
	cmd.AddValue("ce", " Client application stop time/s [default 5s]", clientStopTime);
	cmd.AddValue("ss", " Server application start time/s [default 0s]", serverStartTime);
	cmd.AddValue("se", " Server application stop time/s [default 6s]", serverStopTime);
    cmd.AddValue ("verbose", "Turn on all device log components", verbose);
	cmd.Parse(argc, argv);

	if (verbose) {
        LogComponentEnable("HULAFatTreeExample", LOG_LEVEL_INFO);
        LogComponentEnable("RPCApplication", LOG_LEVEL_INFO);
	}
	...
	NodeContainer bridge[num_pod][num_edge];				// NodeContainer for edge bridges, Bridge does not require internet
	for (i = 0; i<num_pod; i++) {
        for (j = 0; j<num_edge; j++) {
            bridge[i][j].Create(num_bridge);
            //internet.Install(bridge[i][j]);
        }
	}
	...
	for (uint32_t client_i = 0; client_i < half_host_num; client_i++)
	{
        ...
        //NS_LOG_INFO( "Parametri server " << s_p << " " << s_q << " " << s_t );
        ...
        if (algorithm == 2) {
            ...
            HulaGeneratorHelper hulaProbeGeneratorHelper;
            hulaProbeGeneratorHelper.SetGeneratorAttribute("Interval", StringValue("1ms"));
            ApplicationContainer hulaApplicationContainer = hulaProbeGeneratorHelper.Install(host[s_p][s_q].Get(s_t));
            hulaApplicationContainer.Add(hulaProbeGeneratorHelper.Install(host[p][q].Get(t)));
            hulaApplicationContainer.Start(Seconds(hulaStartTime));
            hulaApplicationContainer.Stop(Seconds(serverStopTime));
	...
	//
	CsmaHelper csma;
	csma.SetChannelAttribute("DataRate", StringValue(leafDataRate));
	csma.SetChannelAttribute("Delay", StringValue(stringLeafDelay));

    CsmaHelper csmaHighCapacity;
    csmaHighCapacity.SetChannelAttribute("DataRate", StringValue(dataRate));
    csmaHighCapacity.SetChannelAttribute("Delay", StringValue(stringDelay));

	//=========== Connect edge switches to hosts ===========//
	//	
	NetDeviceContainer hostSw[num_pod][num_edge][num_bridge];
	NetDeviceContainer bridgeDevices[num_pod][num_edge][num_bridge];
	Ipv4InterfaceContainer ipContainer[num_pod][num_edge][num_bridge];

	for (i = 0; i<num_pod; i++) {
		for (j = 0; j<num_edge; j++) {
            for (uint32_t f = 0; f < num_bridge; f++) {

                NetDeviceContainer link1 = csmaHighCapacity.Install(NodeContainer(edge[i].Get(j), bridge[i][j].Get(f)));
                hostSw[i][j][f].Add(link1.Get(0));
                bridgeDevices[i][j][f].Add(link1.Get(1));

                NS_LOG_INFO( "Connecting [pod,edge,bridge] [" << i << "," << j << "," << f << "]");

                char *subnet;
                subnet = toString(10, i, j, 0);
                char *defaultGateway = toString(10, i, j, 1);
                address.SetBase(subnet, "255.255.255.0");

                for (h = 0; h < num_host; h++) {
                    NetDeviceContainer link2 = csma.Install(NodeContainer(host[i][j].Get(h), bridge[i][j].Get(f)));
                    hostSw[i][j][f].Add(link2.Get(0));
                    bridgeDevices[i][j][f].Add(link2.Get(1));
                ...
                bHelper.Install(bridge[i][j].Get(f), bridgeDevices[i][j][f]);

                //Assign address
                ipContainer[i][j][f] = address.Assign(hostSw[i][j][f]);
            }
		}
	}
